...
def make_white_transparent(image_path: str, threshold: int = 240) -> None:
    ...
    data = img.getdata()
    img.putdata(
        [(r, g, b, 0) if r >= threshold and g >= threshold and b >= threshold else (r, g, b, a) for r, g, b, a in data]  # type: ignore[misc]
    )
    img.save(image_path)
...
def potatofy_icons(folder: str, max_size: int = 64, max_colors: int = 64) -> None:
    for path in Path(folder).rglob("*_icon.png"):
        with Image.open(path) as img:
            ...
            img = img.convert("P", palette=Image.ADAPTIVE, colors=max_colors)  # type: ignore[attr-defined]
