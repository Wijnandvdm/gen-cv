...
class PDF(FPDF):
    ...
    def render_experience_tldr(self, section):
        ...
        for item in section.section_content:
            ...
            self.draw_bullets(lines=item.details.bullets, x=self.outlined_x, base_indent=30)
            self.ln(self.layout.spacing.section_gap)
    ...
    def render_experience(self, section):
        ...
        for item in section.section_content:
            ...
            if item.details.description:
                self.draw_bullets(lines=item.details.description, x=self.outlined_x, base_indent=30)
            self.ln(self.layout.spacing.section_gap)
    ...
    def render_projects(self, section):
        ...
        for item in section.section_content:
            ...
            self.ensure_page_space(img_size)  # header + image must stay together
